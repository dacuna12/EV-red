...
def current_unbalance(sim, phase_ids, unbalance_type="NEMA", type=None):
    ...
    elif unbalance_type == "NEMA_ponderado":
        return _nema_ponderado(sim,phase_ids)
    elif unbalance_type == "componentes_inversa":
        return _componente_inversa_corriente(sim, phase_ids)
    elif unbalance_type == "NEMA_MG1":
        return _nema_current_unbalance_MG1(sim, phase_ids)
    else:
        raise ValueError("type must be NEMA not {0}".format(unbalance_type))

def _componente_inversa_corriente(sim,phase_ids):
    """
        Calcula el desbalance con el calculo propuesto por Fernando g.T*M*g que es proporcional a la componente inversa
        de la corrienes
    """
    # Levanto las corrientes
    ...
    unbal = np.zeros(np.size(currents,1))
    # Armo la matriz M
    M = np.array([[1, -1 / 2, -1 / 2],
                  [-1 / 2, 1, -1 / 2],
                  [-1 / 2, -1 / 2, 1]])
    for t in range(0,np.size(currents,1)):
        unbal[t] = currents[:,t].T @ M @ currents[:,t]
    return unbal

def _nema_ponderado(sim, phase_ids):
    """ Calcula el desbalance como el indicador habitual del NEMA pero sin dividir por el promedio

    """
    currents_dict = constraint_currents(sim, constraint_ids=phase_ids)
    currents = np.vstack([currents_dict[phase] for phase in phase_ids])
    return (np.max(currents, axis=0) - np.mean(currents, axis=0))
...
def _nema_current_unbalance_MG1(sim, phase_ids):
    """ Calcula el indicador NEMA de la norma MG1-14.34 - Es una variacion del anterior que para mi estaba mal implementado
    ...
    Args:
        sim (Simulator): simulacion corrida
        phase_ids (List[str]): identificador de la fase

    Returns:
        List[float]: numpy con el indicador de desbalance para cada fase
    """
    currents_dict = constraint_currents(sim, constraint_ids=phase_ids)
    currents = np.vstack([currents_dict[phase] for phase in phase_ids])
    currents_prom = np.sum(currents, axis=0)/3
    return np.nan_to_num((np.max(np.abs(currents - np.tile(currents_prom,(3,1))), axis=0)) / currents_prom)
